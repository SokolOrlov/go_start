...
func Run(produceCh chan common.UpdateTripStatus, log *slog.Logger, cfg *config.KAFKA) error {
	...
	go func(producer *sarama.SyncProducer, log *slog.Logger) {
		for {
			...
			model := common.KafkaMessage{Type: "UpdateTripStatus", Body: bytes}
			mess := PrepareMessage(cfg.PRODUCER.TOPIC, model)
